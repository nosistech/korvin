...
- **Researches CVEs on demand** — `/patch openssl` returns severity scores and patch recommendations. > **Important:** `/patch` output is generated by an LLM and must be verified against official sources (NVD, vendor advisories) before acting on it. Never apply patches based solely on KORVIN's output. 
...
> **Important:** `/patch` output is generated by an LLM and must be verified against official sources (NVD, vendor advisories) before acting on it. Never apply patches based solely on KORVIN's output.
...
│   │   ├── sandbox.js              # Planned — not yet implemented
│   │   └── auto-patcher.js         # Planned — not yet implemented
│   ├── dashboard/
